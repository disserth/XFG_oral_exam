...
stockprice=xlsread('Copula.xlsx','Tabelle1','B:M');  % stock return daily data
Size=xlsread('Copula.xlsx','Tabelle2','B:B');                % firm size

[T,n]=size(stockprice);
data=log(stockprice(2:T,:)./stockprice(1:(T-1),:));

[T,N]=size(data);
Return=data*100;    % Panel returns series across N SIFI
...
h=250;  % backward period for estimating GARCH residuals
Temp=Return((Y-h):(Y-1),:);
